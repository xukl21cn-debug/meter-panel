...
- **后端接口**(需要能访问采集服务):设置 → 数据来源 → 后端接口,填写后端服务地址即可

> 之前的设置若指向 `127.0.0.1:8787`(旧版模拟服务),会自动迁移为本地内置样例模式;旧版保存的完整 URL 会自动提取出地址,无需手动处理。

## 后端接口(只需一个地址)

面板只使用两个 CSV 接口,**路径与端口固定**,设置里只需填写**后端服务地址**(IP 或域名):
